...
function update() {
    try {
        ...
        document.querySelectorAll("#filter-in-courses input, #filter-in-courses select").forEach(o =>
            o.addEventListener('input', e => {
                console.log("filtrando");
                E.advancedCourseFilter("#filter-in-courses", ".course-table-row");
            }));
        document.querySelectorAll("#filter-in-users-group input, #filter-in-users-group select").forEach(o =>
            o.addEventListener('input', e => {
                console.log("filtrando");
                E.advancedStudentFilter("#filter-in-users-group", ".student-table-row");
            }));
        ...
        const eliminarTodos = document.getElementById("cmModalButton");
        eliminarTodos.addEventListener("click", function () {
            console.log("entre");
            if(eliminarTodos.value === "rm-users"){
                E.deleteAll("#users", "U");
                evtUser.length = 0;
            }
            else if(eliminarTodos.value === "rm-courses"){
                E.deleteAll("#courses", "C");
                evtCourse.length = 0;
            }
            else if(eliminarTodos.value === "rm-professors"){
                E.deleteAll("#professors", "P");
                evtProfessor.length = 0;
            }
            else{
                E.deleteAll("#students", "S");
                evtStudent.length = 0;
            }
        });
        
        let matricula = document.getElementById("matricular");
        matricula.addEventListener("click", function () {
            let selectEdition = document.getElementById("editionSelect");
            let selectedOption = selectEdition.options[selectEdition.selectedIndex].value;
            console.log(selectedOption);
            E.matriculateUsers("#users", selectedOption);
        });

        let boton = document.getElementById('cmDeleteModal');
        boton.addEventListener('click', function(){
            let id = this.value;
            var array = id.split('&');
            if(array[0] === 'U'){
                console.log("Usuario con id: " + array[1]);
                E.bindRmUserRow("#users", array[1]);
            }
            else if(array[0]){
                console.log("Curso con id: " + array[1]);
                E.bindRmCourseRow("#courses", array[1]);
            }
        });
        ...
        E.alternaBusquedaAvanzadaUsuarios("#search-advanced-toggle-course", "#search-in-courses-input", "#filter-in-courses");
        E.alternaBusquedaAvanzadaUsuarios("#search-advanced-toggle-group", "#search-in-students-input", "#filter-in-users-group");
        
        E.bindCheckboxColumn("#users", evtUser);
        E.bindCheckboxColumn("#courses", evtCourse);
        E.bindCheckboxColumn("#professors", evtProfessor);
        E.bindCheckboxColumn("#students", evtStudent);
